...
export default {
  ...
  computed: {
    findings: function () {
      ...
      const directions = this.deviceInfo.directions
      const propTraffic = directions['111'] > 0
      findings.proprietary = propTraffic

      const upTraffic = directions['010'] > 0 || directions['100'] > 0
      const downTraffic = directions['011'] > 0 || directions['101'] > 0

      const sensor = upTraffic && !downTraffic
      findings.sensor = sensor
      const actuator = downTraffic && !upTraffic
      ...
      const passiveDevice = directions['000'] > 0 || directions['001'] > 0 || directions['110'] > 0
